...
```r
c(1, 2, 3, 4)# fungsi C menggabungkan semua component menjadi sebuah vector, namun semuanya menjadi character apabila jenis datanya berbeda beda
```
...
```r
dim(iris) # melakukan pengecekan dimensi dari object iris
```
...
```r
str(iris) # menampilkan struktur internal dari object , melihat tipe data , dimensi dll
```
...
```r
colnames(iris) # mendapatkan nama kolom dari object iris
```
...
```r
head(iris) # ___menampilkan bagian awal object secara default mencetak 6 baris pertama
```
...
```r
head(iris, 10) # ___menampilkan 10 baris pertama dari object
```
...
```r
tail(iris) # secara default menampilkan 6 baris akhir dari object
```
...
```r
tail(iris, 10) # menampilkan 10 baris akhir dari object
```
...
```r
summary(iris) # menampilkan rangkuman dari data data yang ada pada object
```
...
```r
iris[c(1, 6, 12),] # c berarti semua kolom , dan yang hanya dicetak adalah baris 1 , 6 dan 12
```
...
```r
iris[ ,2, drop = FALSE] # subset atau ekstrak data hanya pada kolom 2 dan namun semua baris, namun datanya akan menjadi vector , untuk menjaga tetap menjadi data frame maka ditambahkan perintah drop = FALSE
```
...
```r
iris[2,2] # menampilkan data dari iris baris ke 2 kolom ke 2
```
...
```r
iris$Sepal.Length # ekstrak data pada kolom 'Sepal.Length', fungsi dolar $ dapat digunakan untuk menggantikan drop = FALSE
```
